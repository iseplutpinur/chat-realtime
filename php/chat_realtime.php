...
class Chat_realtime
{
	...
	function send_message($name, $ke, $message, $date, $avatar, $tipe, $nama_file, $is_file, $url_file)
	{
		$nama_file = $is_file ? $nama_file : "";
		$url_file = $is_file ? $url_file : "";
		$data = array();
		$sql = $this->dbh->prepare("INSERT INTO messages (name,ke,avatar,message,tipe,date,nama_file,is_file,url_file) VALUES (?,?,?,?,?,?,?,?,?)");
		$sql->execute(array($name, $ke, $avatar, $message, $tipe, $date, $nama_file, $is_file, $url_file));
		$data['status'] = 'success';
